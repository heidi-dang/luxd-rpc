...
describe('RpcClient', function() {
  ...
  it('accept many values for bool', function(done) {
    ...
    async.eachSeries([true, 'true', 1, '1', 'True'], function(i, next) {
      client.importAddress('LMd9297fnu6ik1R8A3bqoDazuAyKohRuTn', '', i, function(error, parsedBuf) {
        should.not.exist(error);
        should.exist(parsedBuf);
  ...
  it('should process object arguments', function(done) {
    ...
    var obj = {'LMd9297fnu6ik1R8A3bqoDazuAyKohRuTn': 1};
    async.eachSeries([obj, JSON.stringify(obj)], function(i, next) {
      client.sendMany('account', i, function(error, parsedBuf) {
        should.not.exist(error);
        should.exist(parsedBuf);
        parsedBuf.params[1].should.have.property('LMd9297fnu6ik1R8A3bqoDazuAyKohRuTn', 1);
        next();
      });
    }, function(err) {
      requestStub.restore();
  ...
  it('should handle connection rejected 401 unauthorized', function(done) {
    ...
    client.getBalance('LMd9297fnu6ik1R8A3bqoDazuAyKohRuTn', 6, function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('LUX JSON-RPC: Connection Rejected: 401 Unnauthorized');
      done();
    });
  ...
  it('should handle connection rejected 401 forbidden', function(done) {
    ...
    client.getDifficulty(function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('LUX JSON-RPC: Connection Rejected: 403 Forbidden');
      done();
    });
  ...
  it('should handle 500 work limit exceeded error', function(done) {
    ...
    client.getDifficulty(function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('LUX JSON-RPC: Work queue depth exceeded');
      done();
    });
  ...
  it('should handle EPIPE error case 1', function(done) {
    ...
    client.getDifficulty(function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('LUX JSON-RPC: Request Error: write EPIPE');
      done();
    });
  ...
  it('should handle ECONNREFUSED error', function(done) {
    ...
    client.getDifficulty(function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('LUX JSON-RPC: Request Error: connect ECONNREFUSED');
      done();
    });
  ...
  it('should callback with error if invalid json', function(done) {
    ...
    client.getDifficulty(function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('LUX JSON-RPC: Error Parsing JSON: Unexpected token o');
      done();
    });
  ...
  it('should callback with error if blank response', function(done) {
    ...
    client.getDifficulty(function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('LUX JSON-RPC: Error Parsing JSON: Unexpected end of input');
      done();
    });
